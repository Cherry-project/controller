...
public class LaunchPrimitive {
	...
	public static int startBehaviorPrimitive(String behavior, String url){
		...
		try {
			...
			try {
				logger.info("Play Behave Primitive: " + behavior);
				status = HttpConnection.sendGetCode(url + "/primitive/" + behavior + "/start.json");
			} catch (Exception e) {
				// TODO Auto-generated catch block
	...
	public static void stopAPrimitive(String behavior){
	...
	// Method to stop "behave" primitives with a robot target
	public static void stopPrimitive(String url){
		
		try {
			logger.info("Stop Behave Primitive");
			String current_primitive = HttpConnection.sendGet(url + "/primitive/running/list.json");
			System.out.println(current_primitive);
			JSONObject jsonObject = new JSONObject(current_primitive);
			HttpConnection.sendGet(url + "/primitive/" + jsonObject.getJSONArray("running_primitives").getString(0) + "/stop.json");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	// Method to start "speak" primitive with a robot target
